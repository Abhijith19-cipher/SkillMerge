    ...
    <>
      ...
      <BentoCardGrid gridRef={gridRef}>
        {items.map((card, index) => {
          ...
          // Parse color string to rgb values for glow if it's hex, or use default
          let cardGlowColor = glowColor;
          if (card.tagColor) {
             // Basic hex to rgb conversion if it's a hex color
             const hex = card.tagColor.replace('#', '');
             if (hex.length === 6) {
               cardGlowColor = `${parseInt(hex.slice(0, 2), 16)}, ${parseInt(hex.slice(2, 4), 16)}, ${parseInt(hex.slice(4, 6), 16)}`;
             }
          }
          ...
          const cardProps = {
            ...
            style: {
              backgroundColor: card.color || 'rgba(15, 0, 32, 0.6)',
              '--glow-color': cardGlowColor,
              borderTop: `3px solid ${card.tagColor || '#8400ff'}`
            }
          };
            ...
            return (
              ...
              >
                ...
                <div className="magic-bento-card__header">
                  <span className="magic-bento-card__label" style={{ color: card.tagColor, borderColor: `${card.tagColor}55` }}>{card.label}</span>
                  <span style={{ fontFamily: 'monospace', fontSize: '0.7rem', color: '#6b7280' }}>{card.duration}</span>
                </div>
                <div className="magic-bento-card__content">
                  ...
                  <div style={{ marginTop: 'auto', paddingTop: '1rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                     <span style={{ fontFamily: 'monospace', fontSize: '0.7rem', color: '#6b7280' }}>📚 {card.hours}</span>
                     <span style={{ fontFamily: 'monospace', fontSize: '0.7rem', color: card.tagColor, opacity: 0.8 }}>
                        {'> details →'}
                     </span>
                  </div>
                </div>
