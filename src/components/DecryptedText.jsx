        ...
        if (sequential) {
          if (direction === 'forward') {
            if (prevRevealed.size < text.length) {
              const newRevealed = new Set(prevRevealed); newRevealed.add(getNextIndex(prevRevealed));
          ...
          if (direction === 'reverse') {
            if (pointerRef.current < orderRef.current.length) {
              const newRevealed = new Set(prevRevealed); newRevealed.delete(orderRef.current[pointerRef.current++]);
          ...
          if (direction === 'reverse') {
            ...
            const nextSet = removeRandomIndices(currentSet, Math.max(1, Math.ceil(text.length / Math.max(1, maxIterations))));
